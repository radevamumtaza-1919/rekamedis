...
class Form_permintaan_klinik_model extends CI_Model {
    ...
    public function get_today_formulir(){
        $this->db->where('DATE(tgl_form)', date('Y-m-d'));
        return $this->db->get('form_permintaan_klinik')->result();
    }

    public function get_formulir_by_id($id){
        $this->db->select('form_permintaan_klinik.*, pasien.no_telp as no_telp, pasien.alamat as alamat, form_permintaan_klinik.nama_pasien');
    ...
    {
        $this->db->select('form_permintaan_klinik.*, pasien.no_telp as no_telp, pasien.alamat as alamat, form_permintaan_klinik.nama_pasien as nama_pasien');
        $this->db->from('form_permintaan_klinik');
        $this->db->join('pasien', 'pasien.id_pasien = form_permintaan_klinik.id_pasien', 'left');
        $this->db->where('form_permintaan_klinik.id', $id);
        return $this->db->get()->row();
